...
from site_common import esc, header, local_url
...
def render_news(items: list[dict], prefix: str = "") -> str:
    rows = []
    for item in items[:5]:
        ...
        rows.append(
            ...
            f'<div class="news-date"><time datetime="{esc(item.get("date", ""))}">{esc(display_date)}</time>{new_badge}</div>'
            f'<p>{title}</p></article>'
        )
...
def build_news_index(data: dict, news_items: list[dict]) -> str:
    ...
    for item in news_items:
        ...
        rows.append(
            ...
            f'<div class="news-archive-date"><time datetime="{esc(item.get("date", ""))}">{esc(display_date)}</time>{new_badge}</div>'
            f'<div><h2><a href="../{esc(item["url"])}">{esc(item.get("title", ""))}</a></h2>{summary}</div></article>'
        )
    body = "".join(rows) or '<p class="news-empty">公開中のお知らせはありません。</p>'
    canonical = urljoin(site["url"], "news/")
    return f'''<!doctype html><html lang="ja"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>News | {esc(p["nameJa"])} / {esc(p["nameEn"])}</title>
<meta name="description" content="{esc(p["nameJa"])}の最新情報・お知らせ。"><meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1">
<link rel="canonical" href="{esc(canonical)}"><link rel="icon" href="../assets/favicon.svg" type="image/svg+xml"><link rel="alternate icon" href="../assets/noto-lab-icon.png" type="image/png">
<link rel="stylesheet" href="../styles.css"><script src="../script.js" defer></script></head><body>
...
<section class="news-archive"><div class="container news-archive-inner">{body}</div></section></main>
...
<title>{esc(item.get("title", ""))} | News | {esc(p["nameJa"])}</title><meta name="description" content="{esc(item.get("summary") or item.get("title", ""))}"><meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1"><link rel="canonical" href="{esc(canonical)}">
<link rel="icon" href="../assets/favicon.svg" type="image/svg+xml"><link rel="alternate icon" href="../assets/noto-lab-icon.png" type="image/png"><link rel="stylesheet" href="../styles.css"><script src="../script.js" defer></script></head><body>
{header(p, prefix="../", active="home")}<main><article class="article-page"><div class="container article-container"><p class="breadcrumb"><a href="../index.html">Home</a> / <a href="index.html">News</a></p>
<div class="article-meta-row"><time datetime="{esc(item.get("date", ""))}">{esc(display_date)}</time>{new_badge}</div><h1>{esc(item.get("title", ""))}</h1>{lead}<div class="article-body">{body}{related}</div><div class="article-back"><a href="index.html">← News一覧へ戻る</a></div></div></article></main>
